...
int consulta_lista_pos(Lista* li, int pos, struct aluno *al) {
    if (li == NULL || pos >= li->qtd || pos < 0) return 0;
    int i = 0;
    struct aluno *no = &li->dados[i];
    while (no != NULL && i < pos) {
        i++;
        no = &li->dados[i];
    }
...
int consulta_lista_mat(Lista* li, int mat, struct aluno *al) {
    ...
    int i = 0;
    struct aluno *no = &li->dados[i];
    while (no != NULL && no->matricula > 0 && no->matricula != mat) {
        i++;
        no = &li->dados[i];
    }
    if (no == NULL || no->matricula <= 0) return 0;
    else {
        al = no;
        return 1;
    }
